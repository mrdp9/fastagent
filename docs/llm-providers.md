...
FastAgent ships with a mock provider that works offline. Switch to a real
model with an env var or a constructor argument.

## Provider matrix

| Provider | Needs API key? | Base URL | Default model | Setup |
|---|---|---|---|---|
| `mock` | no | (none) | `mock-0` | Default. Always works. |
...
`(fallback to mock: provider 'openai' failed: ...)`.

To disable auto-fallback, replace `client.chat` directly:
...
- Tool-call loop is implemented but not yet wired through `@app.agent` by
  default — see the roadmap in CHANGELOG.md.
...
| Tool calls don't fire | Model doesn't know about the tools | Check the JSON schema was generated (`function_to_tool_schema(fn)`) |
